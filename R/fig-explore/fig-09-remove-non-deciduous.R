...
min_month <- 1 #incl
max_month <- 12 #incl
sites_to_remove <- c("CHL", "EIC", "AL1", "AL2", "AL3", "BOI", "CFI", "TU1", "TU2")
agg_period <- 120
# corr_fig_height <- 9
path_out <- "fig/10-remove-non-deciduous/"
...
for(eli_var in c("TRW", "Kh", "MVA")){
  ...
  dat_var_exp[, prop_variance := round(prop_variance, 3)]
  dat_var_exp[, prop_variance_cumulative := round(prop_variance_cumulative, 3)]
  fwrite(dat_var_exp, file = paste0(path_out, eli_var, "_pca-by-site-variance-explained.csv"))
  
  
  # biplot site
  pdf(paste0(path_out, eli_var, "_pca-by-site-biplot.pdf"),
      width = 10, height = 10)
  biplot(pca_site)
  dev.off()
  ...
  ggsave(paste0(path_out, eli_var, "_map-comparison-PC.pdf"),
         gg_out,
         width = 12, height = 6)
