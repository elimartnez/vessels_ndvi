...
min_month <- 3 #incl
max_month <- 10 #incl
sites_to_remove <- c() # c("CHL", "EIC", "BOI")
agg_period <- 120
# corr_fig_height <- 9
path_out <- "fig/14-check-pca-full/"
...
for(eli_var in c("TRW", "Kh", "MVA")){
  ...
  ggsave(gg_pca_cor,
         file = paste0(path_out, eli_var, "_pcaCorr-corr-plot.pdf"), 
  ...
  pca_site <- prcomp(t(mat_pca_site), rank. = 10)
  ...
  ggsave(paste0(path_out, eli_var, "_pcaSite-map-comparison-PC.png"),
         gg_out,
         width = 12, height = 6)
