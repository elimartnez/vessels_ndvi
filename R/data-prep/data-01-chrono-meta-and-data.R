...
fn <- "data-eli/v5/MODIFIED_chronologies from the others_4.xlsx"
...
l_data <- foreach(i_sheet = excel_sheets(fn)[-1]) %do% {
  dat <- data.table(read_excel(fn, i_sheet))
  setnames(dat, 1, "year")
...
dat_others[, .(site, year, TRW, MVA, Kh)] %>% 
  melt(id.vars = c("site", "year")) -> dat_others_long
...
saveRDS(dat_chrono, file = "data/chrono-01.rds")
