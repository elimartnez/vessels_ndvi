...
min_month <- 1 #incl
max_month <- 12 #incl
sites_to_remove <- c("CHL", "EIC")
agg_period <- 120
# corr_fig_height <- 9
path_out <- "fig/08-refined4-after-qc-modis/"
...
for(eli_var in c("TRW", "Kh", "MVA")){
  ...
  pca_site <- prcomp(mat_pca_site, rank. = 10)
  ...
  dat_pc <- data.table(site = sites, pca_site$x)
...
data.table(site = sites, pca_site$x[, 1:3]) %>% 
  merge(dat_meta) %>% 
...
# correlate PC cor plots vs site plots ------------------------------------
...
l_gg <- dat_cor_site_pca[, .(cor(pc_val, estimate)), .(site, variable)] %>% 
  split(by = "variable") %>% 
  lapply(function(x){
    x %>% 
      ggplot(aes(fct_reorder(site, V1), V1))+
      geom_point()+
      facet_wrap(~variable)+
      theme_bw()
  })

patchwork::wrap_plots(l_gg, nrow = 2)

dat_cor_site_pca[, .(cor(pc_val, estimate)), .(site, variable)] %>% 
  ggplot(aes(fct_reorder(site, V1), V1))+
  geom_point()+
  facet_wrap(~variable)+
  theme_bw()
...
dat_plot_tile %>% 
  ggplot(aes(variable, site2, fill = V1))+
  geom_tile()+
  scale_fill_gradient2()+
  theme_bw()
...
gg_loading <- data.table(site = sites, pca_site$x[, 1:3]) %>% 
  merge(dat_meta) %>% 
  melt(measure.vars = paste0("PC", 1:3)) %>% 
  ggplot(aes(lon, lat, colour = value))+
  borders()+
...
ggsave("fig/09-pc/map-comparison.pdf",
       gg_out,
       width = 12, height = 6)
